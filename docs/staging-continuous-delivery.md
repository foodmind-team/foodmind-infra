...
5. Systems Manager refetches the tested Infra SHA while bypassing any stale
   commit graph, creates an immutable Git worktree on EC2, and runs
   `scripts/cd/deploy-staging.sh`. This lets a partial clone recover missing
   objects without changing the tested revision.
